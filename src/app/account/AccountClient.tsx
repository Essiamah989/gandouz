...
export default function AccountClient({ user, orders, wishlist }: { user: any, orders: any[], wishlist: any[] }) {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "DELIVERED":
        return { label: "Livré", class: "bg-green-100 text-green-700" };
      case "CANCELLED":
        return { label: "Annulé", class: "bg-red-100 text-red-700" };
      case "PROCESSING":
        return { label: "En préparation", class: "bg-blue-100 text-blue-700" };
      case "SHIPPED":
        return { label: "Expédié", class: "bg-purple-100 text-purple-700" };
      case "PENDING":
      default:
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">{user?.email || "Non renseigné"}</p>
            ...
            <div className="bg-[#06091F] text-white rounded-3xl p-6 shadow-sm relative overflow-hidden">
              ...
              <p className="text-xs text-white/60 mt-2 leading-relaxed">
                Équivalent à <strong>{(user?.loyaltyAcc?.balance ?? 0).toFixed(3)} TND</strong> de remise. Les points sont utilisables directement lors de vos prochains paiements.
              </p>
              
              <div className="mt-6 pt-4 border-t border-white/10 space-y-2">
                <p className="text-[10px] text-white/40 uppercase tracking-widest font-semibold">Historique des points</p>
                {!user?.loyaltyAcc?.transactions || user.loyaltyAcc.transactions.length === 0 ? (
                  <p className="text-xs text-white/40 italic">Aucune transaction enregistrée pour l'instant.</p>
                ) : (
          ...
          {/* Section Commandes & Coups de Cœur */}
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-6 lg:p-8 shadow-sm">
              ...
              {orders.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm font-semibold text-gray-400">Aucune commande passée pour l'instant.</p>
                  <p className="text-xs text-gray-400 mt-1">Prêt à découvrir notre sélection ? Parcourez notre catalogue !</p>
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${statusInfo.class}`}>
                              {statusInfo.label}
                            </span>
                          </div>
                          <p className="text-xs text-gray-500">
                            {o.items?.length || 1} article{(o.items?.length || 1) > 1 ? "s" : ""} · {Number(o.total).toLocaleString('fr-FR')} TND
                          </p>
                        ...
                        >
                          Suivre l'état <ExternalLink className="w-3.5 h-3.5" />
                        </Link>
                      </div>
                    );
                  })}
            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-6 lg:p-8 shadow-sm">
              ...
              {wishlist.length === 0 ? (
                <p className="text-sm font-semibold text-gray-400 text-center py-6">Votre liste de favoris est actuellement vide.</p>
              ) : (
